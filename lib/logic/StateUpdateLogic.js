...
class StateUpdateLogic {
    ...
    _checkFirstClientIdentifierValidation(identifier, tcpClientId, udpClientId){
        ...
        if(validated >= total){
            ...
            this._sendToGroup({
                ...
                groupId: this.groupId
            }, this._server.PACKET_HEADERS.MESSAGE_UPDATE);
    ...
    handleUdpMessage(event, payload){
        ...
        switch(event){
            ...
            case "message":
                this._handleMessageUpdate(payload.tid, payload, "udp");
                break;

            case "world":
                this._handleWorldUpdate(payload.tid, payload, "udp");
                break;
  ...
  /**
   * handles tcp update types: message and world
   * (not state, since that is handled in roomgs directly)
   * @param event
   * @param payload
   */
    handleTcpMessage(event, payload){

      const clientId = payload.tcpId;
      this.log(`received tcp-update-packet from ${clientId} as event: ${event}.`);

      //no validation needed since packet comes from tcp connection

      switch(event){

        case "message":
          this._handleMessageUpdate(clientId, payload, "tcp");
          break;

        case "world":
          this._handleWorldUpdate(clientId, payload, "tcp");
          break;
    ...
    _getServerForType(type){
        switch(type){
          case "tcp": return this._getTcp();
          case "udp": return this._getUdp();
          default: throw new Error("unknown server type: " + type);
        }
    }

    _handleWorldUpdate(clientId, payload, type){

      //check if this message is a simple broadcast
      if(payload.bcast){
        payload.clientId = clientId; //for security reasons only the bcast member is forwarded
        return this._sendToGroup(payload.bcast, this._server.PACKET_HEADERS.WORLD_UPDATE);
      }

      //TODO handle others
    }

    _handleMessageUpdate(clientId, payload, type){

      //check if this message is a simple broadcast
      if(payload.bcast){
        payload.clientId = clientId; //for security reasons only the bcast member is forwarded
        return this._sendToGroup(payload.bcast, this._server.PACKET_HEADERS.MESSAGE_UPDATE);
      }
    ...
    _loop(){
        this._state.getStatesAsList().then(states => {
            ...
            this._getTcp().sendList(clientIds, {
                type: INTERNAL,
                header: this._server.PACKET_HEADERS.STATE_UPDATE,
                content: {
                    created: Date.now(),
