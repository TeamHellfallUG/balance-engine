...
class Server extends EventEmitter {
    ...
    constructor({host, port, redis, log, engine, debug}){
        ...
        switch(engine){
            ...
            case "uws":
                ...
                this.wss.on("error", error => {
                    this.log("tcp server error occured: " + error.message);
                    super.emit("error", error);
                });
                break;

            default:
                throw new Error("engine " + engine + " is not supported. Available: [uws]");
        }
        ...
        this.log(`tcp server up @ ${host}:${port} engine: ${engine} as origin: ${this.originId}.`);
    }
