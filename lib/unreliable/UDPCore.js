...
const WSS_MESSAGES_TOPIC_OUT = "udpws:messages:topic:out";
const WSS_MESSAGES_TOPIC_GLOBAL = "udpws:messages:topic:global";

class UDPCore extends EventEmitter {
    ...
    constructor({host, port, redis, log, engine, debug}){
        ...
        switch(engine){
            ...
            default:
                throw new Error("engine " + engine + " is not supported. Available: [udpws]");
        }
        ...
        this.log(`udp server up @ ${host}:${port} engine: ${engine} as origin: ${this.originId}.`);
    }

    getInfo(){
        const stats = JSON.parse(JSON.stringify(this._stats));
        stats.server.clients = this.clients.length;
        return stats;
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            socket.send(message, err => {
                ...
                if(err){
                    ...
                    this._stats.packet.err++;
                    return reject(err);
                }

                this._stats.packet.out++;
