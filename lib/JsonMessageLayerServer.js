    ...
    _attachMessageListener(){
        ...
        super.on("message", (clientId, message) => {
            ...
            try {
                ...
                return super.log("failed to parse message: " + e.message);
            }

            if(msg.type === INTERNAL){

                //ensure any client that connects to a server
                //with enabled json message layer
                //receives its client-id (emitted by Server.js onSocket)
                if(msg.header === "CONNECTED"){
                    super.log(`received internal connected event, sending identification packet to client ${clientId}.`);
                    this.send(clientId, INTERNAL, {
                        id: clientId
                    }, "J:IDENTIFICATION");
                }
